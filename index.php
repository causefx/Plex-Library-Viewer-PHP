...
$host = "hostname:port"; //up to you to include port or not - Dont use localhost or etc, it will prevent images from loading... also dont inlcude http(s)
$token = "xxxxxxxxxxxxxx"; //plex token
...
$actarray = array
   (
   array("newest","all","recentlyAdded", "recentlyViewed"),//Action Without Current - Opposites
   array("Newest Released Movies","All Movies","Recently Added Movies", "Recently Viewed Movies")//Title
   );
...
<?php foreach ($actarray[0] as $action) {
    ...
    echo '<h4><strong><a href="?act='.$action.'" style="color: rgb(0,255,0)"><font color="FF00CC">Switch to '.ucfirst($linktitle).'</font></a></strong></h4>';
}?>
      ...
      <!-- CONTENT -->
<?php foreach($achxml->Video AS $child) {
	echo '<tr class="gradeA">';    
	echo '<td><center><a href="#myModal'.$child->Media['id'].'" data-toggle="modal"><img src="'.$imgurl.$child['thumb'].$imgurlend.'"></a></center></td>';
